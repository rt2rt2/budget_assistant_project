...
#
# Программа хранит расходы пользователя за месяц (31 день) и способна
# за O(1) отвечать на вопрос "сколько потрачено с дня A по день B?".
# Расходы задаются в виде "день, сумма, категория"
#
# В задании используются:
#   Стек для отмены последнего добавленного расхода
#   Префиксные суммы, для ответа за O(1) на интервальный запрос
#   Бинарное дерево поиска для хранения расходов и вывода всех трат по возрастанию
#   Сортировка вставками для категорий по сумме трат
#   Линейный поиск для поиска дня с максимальным расходом
